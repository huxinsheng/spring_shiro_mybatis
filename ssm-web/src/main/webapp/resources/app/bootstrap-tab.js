var addTabs = function (options) {
    ...
    if(options.url.lastIndexOf('analysis')!=-1){
        if($("#" + id)[0])
            closeTab(id);
    }
    //如果TAB不存在，创建一个新的TAB
    if (!$("#" + id)[0]) {
        ...
        if (options.content) {
            ...
            content = '<div role="tabpanel" class="tab-pane" id="' + id + '"><iframe id="iframe_' + id + '" src="' + options.url + '" width="100%" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="yes" allowtransparency="yes"></iframe></div>';
        }
        //加入TABS
    ...
    $("#" + id).addClass("active");
    $('#' + options.target).parent().addClass('active');
};
