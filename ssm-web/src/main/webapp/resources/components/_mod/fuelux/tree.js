...
(function (factory) {
    ...
    Tree.prototype = {
        ...
        selectedAllItems: function () {
            var $sel = this.$element.find('.tree-selected:visible');
            var data = [];
            $.each($sel, function (index, value) {
                data.push($(value).data());
                var $parentSel = $sel.parent().prev('.tree-branch-header');
                if ($parentSel) {
                    $.each($parentSel, function (i, v) {
                        var d = $(v).data();
                        if (!Tree.contains(data, d)) {
                            data.push(d);
                        }
                    });
                }
        ...
        checkAllItem: function (checked) {
            ...
            if (checked) {
                $sel.find('.tree-item-name >.fa-times').each(function () {
                    var $item = this;
                    $($item).trigger('click');
                });
            } else {
                $sel.find('.tree-item-name >.fa-check').each(function () {
                    var $item = this;
                    $($item).trigger('click');
                });
            }
    ...
    Tree.contains = function (array, obj) {
        var exists = false;
        if (array) {
            $.each(array, function (i, item) {
                if (Tree.compare(item, obj)) {
                    exists = true;
                    return false;
                }
            });
    ...
    Tree.compare = function (objA, objB) {
        if (!Tree.isObj(objA) || !Tree.isObj(objB)) return false; //判断类型是否正确
        if (Tree.getLength(objA) != Tree.getLength(objB)) return false; //判断长度是否一致
        return Tree.compareObj(objA, objB, true);//默认为true
    };
    Tree.compareObj = function (objA, objB, flag) {
        for (var key in objA) {
            if (!flag) //跳出整个循环
                break;
            ...
            if (!Tree.isArray(objA[key])) { //子级不是数组时,比较属性值
                if (objB[key] != objA[key]) {
                    flag = false;
                    break;
                }
            } else {
                if (!Tree.isArray(objB[key])) {
                    flag = false;
                    break;
                }
                ...
                for (var k in oA) {
                    if (!flag) //这里跳出循环是为了不让递归继续
                        break;
                    flag = Tree.compareObj(oA[k], oB[k], flag);
                }
            }
        }
        return flag;
    };
    Tree.isObj = function (object) {
        return object && typeof (object) == 'object' && Object.prototype.toString.call(object).toLowerCase() == "[object object]";
    };
    Tree.isArray = function (object) {
        return object && typeof (object) == 'object' && object.constructor == Array;
    };
    Tree.getLength = function (object) {
        var count = 0;
